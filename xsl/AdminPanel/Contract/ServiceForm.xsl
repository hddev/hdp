...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:variable name="status" select="@status" />
    	...
    	<table cellspacing="0" cellpadding="0" border="0">
    		<thead>
    			<th colspan="10">Редактирование карточки услуги</th>
    		</thead>
    		<!-- <tr>
    			<td class="first"></td>
    		...
    		<tr>
    			...
    			<td>
					<select name="status">
					<xsl:if test="(0 = @status) or ('' = @status)"> 
    	...
    	<input type="hidden" name="action" value="service-edit" />
    	<input type="hidden" name="servicegroup_id" value="{@servicegroup_id}" />
    	</form>
    	</div>
    </xsl:template>
